...
\fontsize{8}{9.6} \selectfont
\DontPrintSemicolon
\KwIn{Modified netlist (after DCC insertion), critical path $p$ }
\KwOut{Lifetime}
\Begin{
Upper Bound $U$\;
Lower Bound $L$\;
Median~$M$\;
	\While{ $U - L < 10^{-3}$}
	{	$M = \frac{U + L}{2}$ \;
		//Derive $R_{i}$, i.e., $X_{i}$ in the regression equation\;
		$R_{i} = 0.0039 \times \left(0.5 \times 86400 \times 365 \times M \right)^{0.2}$\;
		//Predict $R_{p}$, i.e., $Y_{p}$ in the regression equation\;
		$R_{p} = $Predict $p's$ aging rate by the regression of $p$ on $i$\;
		$S_{p} = $Estimate $p's$ slack considering an aging rate of $R_{p}$\;
		\eIf{ $S_{p} > 0$}{  
			$L =  M$
		}{
			$U =  M$
...
\end{comment}
%-------------------------- Paragraph --------------------------------------------------------------------------------------
After we obtain a solution from foregoing steps, we have to estimate the lifetime of the entire circuit. The detailed procedures are shown in Algorithm~\ref{LT_1}. 
