...
\end{equation}
which indicates the timing slack must be greater than zero; otherwise, the design can not work normally. Suppose that, the attacker inserts a 20\% DCC and an 80\% DCC at the inputs of buffer 6 and buffer 3, respectively. Over a period, $C_{i}$ will gain greater than $C_{j}$ does. This way, the timing slack gradually decreases, such that setup-time violations are likely to occur on this critical path, causing the failure of the design circuit.